...
shared(msg) actor class NFToken(
    ...
    private var _toWrap = HashMap.HashMap<Nat, Principal> (1, Nat.equal, Hash.hash);

    //let ICPUNK = actor "qcg3w-tyaaa-aaaah-qakea-cai" : actor {
    let ICPUNK = actor "rwlgt-iiaaa-aaaaa-aaaaa-cai" : actor {
    ...
    public shared(msg) func packageTokenData(t : Text, i : TokenIndex):  async TokenIdentifier {
        return ExtCore.TokenIdentifier.fromText(t,i);
    };


   public shared(msg) func unPackageTokenData(t : TokenIdentifier):  async TokenIndex {
        return ExtCore.TokenIdentifier.getIndex(t);
   };

   public shared(msg) func decodeTokenData(t : TokenIdentifier):  async TokenObj {
        return ExtCore.TokenIdentifier.decode(t);
   };

    //validate ownership of tokenid, and put it in _toWrap.
    public shared(msg) func wrap(tokenid : TokenIdentifier) : async Bool {
        if (ExtCore.TokenIdentifier.isPrincipal(tokenid, Principal.fromActor(ICPUNK)) == false) {
            Debug.print("not exact Principal");
    		return false;
    	};
    	let token = ExtCore.TokenIdentifier.getIndex(tokenid);
    	let owner = await ICPUNK.owner_of(Nat32.toNat(token));
        if (Option.isSome(tokens.get(Nat32.toNat(token)))) {
    			return false;
        };
        if (Principal.equal(owner, msg.caller)) {
          _toWrap.put(Nat32.toNat(token), msg.caller);
          return true;
        } else {
          return false;
        };
    ...
    //this call occurs after user transfer his icpunk tokens to this canister.
    public shared(msg) func mint(tokenid : TokenIdentifier, metadata: TokenMetadata) : async Bool {
        if (ExtCore.TokenIdentifier.isPrincipal(tokenid, Principal.fromActor(ICPUNK)) == false) {
    			return false;
    	};
    	let token = ExtCore.TokenIdentifier.getIndex(tokenid);
        let owner = await ICPUNK.owner_of(Nat32.toNat(token));
        switch(_toWrap.get(Nat32.toNat(token))) {
          case(?p) {
            if (Principal.equal(owner, Principal.fromActor(this))) {
                let tokenInfo: TokenInfo = {
                            index = Nat32.toNat(token);
                            var owner = p;
                            var metadata = ?metadata;
                            var operator = null;
                            timestamp = Time.now();
                };
                tokens.put(Nat32.toNat(token),tokenInfo);
                _addTokenTo(p, Nat32.toNat(token));
                _toWrap.delete(Nat32.toNat(token));
                return true;
            } else {
               Debug.print("owner not this canister");
              return false;
            };
          };
          case(_) {
    ...
    // burn the wrapped token, and transfer original asset to user.
    public shared(msg) func unwrap(tokenid : TokenIdentifier) : async Bool {
        if (ExtCore.TokenIdentifier.isPrincipal(tokenid, Principal.fromActor(this)) == false) {
    			return false;
    	};
    	let token = ExtCore.TokenIdentifier.getIndex(tokenid);
        switch(tokens.get(Nat32.toNat(token))) {
          case(?tokenInfo) {
            if(Principal.equal(tokenInfo.owner, msg.caller) == false) {
    		    return false;
    		};
    		_burn(msg.caller, Nat32.toNat(token));
    ...
    // public update calls
    // TODO: wrap ICPUNKs, user transfer ICPUNK to this canister, then generate an id for it
    ...
    // TODO: unwrap ICPUNK, transfer ICPUNK to its owner

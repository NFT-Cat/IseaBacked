...
import WrapperUtils "./wrapperUtils";
...
shared(msg) actor class NFToken(
    ...
    type TokenIdentifier = WrapperUtils.TokenIdentifier;
    ...
    public shared(msg) func wrap(tokenid : TokenIdentifier) : async Bool {
        if (WrapperUtils.TokenIdentifier.isPrincipal(tokenid, Principal.fromActor(ICPUNK)) == false) {
    			return false;
    	};
    	let token = WrapperUtils.TokenIdentifier.getIndex(tokenid);
    ...
    public shared(msg) func unwrap(tokenid : TokenIdentifier) : async Bool {
        if (WrapperUtils.TokenIdentifier.isPrincipal(tokenid, Principal.fromActor(this)) == false) {
    			return false;
    	};
    	let token = WrapperUtils.TokenIdentifier.getIndex(tokenid);
